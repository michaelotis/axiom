...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zCSTL spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zCSTL without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCSTL spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcstl");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zCSTL mints are not already known
...
int64_t GetPOWBlockValue(int nHeight)
{
...
// Requires cs_main.
void Misbehaving(NodeId pnode, int howmuch, const char *why)
{
    if (howmuch == 0)
        return;
    ...
    if (state->nMisbehavior >= banscore && state->nMisbehavior - howmuch < banscore) {
        LogPrintf("Misbehaving: %s (%s: %d -> %d) BAN THRESHOLD EXCEEDED\n", state->name, why, state->nMisbehavior - howmuch, state->nMisbehavior);
        state->fShouldBan = true;
    } else
        LogPrintf("Misbehaving: %s (%s: %d -> %d)\n", state->name, why, state->nMisbehavior - howmuch, state->nMisbehavior);
}
...
void static InvalidBlockFound(CBlockIndex* pindex, const CValidationState& state)
{
    ...
    if (state.IsInvalid(nDoS)) {
        ...
        if (it != mapBlockSource.end() && State(it->second)) {
            ...
            if (nDoS > 0)
                Misbehaving(it->second, nDoS, "rejected message");
        }
    }
...
bool ReindexAccumulators(list<uint256>& listMissingCheckpoints, string& strError)
{
    ...
    if (!listMissingCheckpoints.empty() && chainActive.Height() >= Params().Zerocoin_StartHeight()) {
        ...
        while (pindex) {
            ...
            if (pindex->nAccumulatorCheckpoint != pindex->pprev->nAccumulatorCheckpoint && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
bool UpdateZCSTLSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZCSTLSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zCSTL supply for block=%s height=%d", __func__,
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zCSTL stake block is less than allowed start height", __func__);

    if (CZCstlStake* zCSTL = dynamic_cast<CZCstlStake*>(stake)) {
        CBlockIndex* pindexFrom = zCSTL->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zCSTL stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zCSTL stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zCSTL->GetValue()));
        if (nChecksum200 != zCSTL->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zCSTL->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZCSTL() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zCSTL fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (!fLiteMode) {
        if (masternodeSync.RequestedMasternodeAssets > MASTERNODE_SYNC_POTENTIAL) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (pfrom->nVersion != 0) {
            ...
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 1, "duplicate version");
            return false;
        }
        ...
        LOCK(cs_main);
        Misbehaving(pfrom->GetId(), 1, "no version");
        return false;
    }
        ...
        if (vAddr.size() > 1000) {
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 20, "msg addr size");
        ...
        if (vInv.size() > MAX_INV_SZ) {
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 20, "msg inv size");
	    ...
	    if (fDebug)
            ...
            if (pfrom->nSendSize > (SendBufferSize() * 2)) {
                Misbehaving(pfrom->GetId(), 50, "send buffer size");
        ...
        if (vInv.size() > MAX_INV_SZ) {
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 20, "message getdata size");
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            if (nDoS > 0)
                Misbehaving(pfrom->GetId(), nDoS, "rejected mempool");
        }
    }
    ...
    else if (strCommand == "headers" && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount > MAX_HEADERS_RESULTS) {
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 20, "headers msg size");
        ...
        BOOST_FOREACH (const CBlockHeader& header, headers) {
            ...
            if (pindexLast != NULL && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                Misbehaving(pfrom->GetId(), 20, "non-contiguous headers seq");
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            ...
            if (!mapBlockIndex.count(block.GetHash())) {
                ...
                if(state.IsInvalid(nDoS)) {
                    ...
                    if(nDoS > 0) {
                        TRY_LOCK(cs_main, lockMain);
                        if(lockMain) Misbehaving(pfrom->GetId(), nDoS, "rejected getblocks");
                    }
                }
    ...
    // getaddr message mitigates the attack.
        ...
        LOCK(cs_main);
        Misbehaving(pfrom->GetId(), 100, "bloom message");
    }
        ...
        if (!filter.IsWithinSizeConstraints()) {
            ...
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 100, "filterload too large");
        } else {
            LOCK(pfrom->cs_filter);
        ...
        if (vData.size() > MAX_SCRIPT_ELEMENT_SIZE) {
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 100, "filteradd too large");
        } else {
            LOCK(pfrom->cs_filter);
            if (pfrom->pfilter)
                ...
                LOCK(cs_main);
                Misbehaving(pfrom->GetId(), 100, "filteradd failure");
            }
        }
    }
