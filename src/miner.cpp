...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zCSTL that have been in mempool long
                    //and higher priority to zCSTL that are large in value
        ...
        if (!fProofOfStake) {
            ...
            FillBlockPayee(txNew, nFees, false);
	...
	if (nAccumBlock < 0) {
        ...
        uint256 hashBlockLastAccumulated = chainActive[nAccumBlock]->GetBlockHash();
        if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
            //For the period before v2 activation, zCSTL will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zcstlTracker->RemovePending(pblock->vtx[1].GetHash());
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //Stake miner main
        if (fProofOfStake) {
            LogPrintf("CastleMiner : proof-of-stake block found %s \n", pblock->GetHash().ToString().c_str());
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zCSTL with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zCSTL key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("CastleMiner(): Signing new block with zCSTL key failed \n");
                    continue;
                }
            } else if (!SignBlock(*pblock, *pwallet)) {
                LogPrintf("CastleMiner(): Signing new block with UTXO key failed \n");
                continue;
            }

            LogPrintf("CastleMiner : proof-of-stake block was signed %s \n", pblock->GetHash().ToString().c_str());
